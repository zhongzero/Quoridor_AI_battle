...
// 		if(vis[x][y])continue;
// 		vis[x][y]=1;
// 		if(x-1>=0&&dis[x-1][y]>dis[x][y]+1&&!H2[x-1][y])dis[x-1][y]=dis[x][y]+1,W.push(pp(x-1,y));
// 		if(x+1<=8&&dis[x+1][y]>dis[x][y]+1&&!H2[x][y])dis[x+1][y]=dis[x][y]+1,W.push(pp(x+1,y));
// 		if(y-1>=0&&dis[x][y-1]>dis[x][y]+1&&!H1[x][y-1])dis[x][y-1]=dis[x][y]+1,W.push(pp(x,y-1));
// 		if(y+1<=8&&dis[x][y+1]>dis[x][y]+1&&!H1[x][y])dis[x][y+1]=dis[x][y]+1,W.push(pp(x,y+1));
// 	}
// }
int Dis1,Dis2;
bool cmpquick;
...
double Getval(bool tp){//evaluate
	...
	double val;
	if(tp==1){
		val=(Dis2-Dis1)+Blocknum1*0.1-Blocknum2*0.1;
		if(Blocknum2)val-=min(2,(H1[X1][Y1]+H1[X1][Y1-1]+H2[X1][Y1]+H2[X1-1][Y1]))*0.2;
		if(Blocknum1)val+=min(2,(H1[X2][Y2]+H1[X2][Y2-1]+H2[X2][Y2]+H2[X2-1][Y2]))*0.2;
		if(cycle<=7){
			if(X1<3)val-=abs(X1-3)*10;
			if(X1>5)val-=abs(X1-5)*10;
		}
		if(cmpquick)val+=(-Dis1)*0.5;
		else val+=Dis2*0.5;
	}
	else {
		// val=Dis2-Dis1;
